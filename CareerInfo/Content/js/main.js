(function ($) { 
	...
	$('.btn-xmind').click(function(){
		if ($('#modal-xmind').length === 0) {
			$('body').append(`<div class="modal fade futuristic-modal" id="modal-xmind" tabindex="-1" aria-labelledby="modal-xmind" aria-hidden="true">
			...
			// 載入 SVG
			$('#modal-xmind .modal-body').load('Content/img/xmind/a1.txt', function () {
				$.getJSON('Content/js/xmindHref.json', function (svgLinks) {
					Object.keys(svgLinks).forEach(function (id) {
						let url = svgLinks[id];
						let $target = $('#' + id);
