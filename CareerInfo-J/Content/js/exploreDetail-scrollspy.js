(function () {
  ...
  const sectionSelector = '.page-content .guide-card[id^="guide"]';
  const svgGuideSelector = '.xmind svg [id^="g"]';
  ...
  function getTargetIdFromSvgId(svgId) {
    // 支援： svg g1 -> guide1
    const match = svgId.match(/^g(\d+)/) || svgId.match(/^content(\d+)/);

    return match ? 'guide' + match[1] : null;
  }

  function bindSvgGuideLinks() {
    document.querySelectorAll(svgGuideSelector).forEach(function (svgGuide) {
      const targetId = getTargetIdFromSvgId(svgGuide.id);

      if (!targetId || !sectionMap[targetId]) return;
      if (svgGuide.getAttribute('data-guide-bound') === 'true') return;

      svgGuide.setAttribute('data-guide-bound', 'true');
  ...
  bindSvgGuideLinks();
  document.addEventListener('xmind:loaded', bindSvgGuideLinks);
